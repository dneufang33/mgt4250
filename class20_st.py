...
with col1:
    profit_ratio_state = df.groupby('State')['Profit'].sum() / df.groupby('State')['Sales'].sum()  # returns sum of profits by state
    profit_ratio_state = profit_ratio_state.reset_index(name="ProfitRatio") # -_> make it a dataframe with reset_index() and name it ProfitRatio
...
with col2:
    ...
    profit_ratio_category = profit_ratio_category.reset_index(name="ProfitRatio") # -_> make it a dataframe with reset_index() and name it ProfitRatio
...
with col3:
    ...
    fig3 = px.line(profit_ratio_category_ym, x="YearMonth", y="ProfitRatio", color="Category")
    st.plotly_chart(fig3, use_container_width=True)

with col4:
    fig4 = px.treemap(df, path = [px.Constant("all"), "Category", "Sub-Category"], values ="Sales", color="Category")
